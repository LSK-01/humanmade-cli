...
let getSim = async (tag, file, creationID) => {
    if (tag != "") {
        let idToken;
        try {
        ...
        console.log("data: ", data, "evidence: ", evidence, "field obj:", evidence.mapValue.fields["mountain%2Bimage%2Fjpeg%2B1708722683111"].stringValue);
        console.log("poop2");
        if (evidence) {
...
yargs(hideBin(process.argv))
    ...
    async handler(argv) {
        ...
        if(inputTags.length != files.length){
            console.log("Number of tags must match number of files");
            return;
        }
        ...
        for (let index = 0; index < files.length; index++) {
            ...
            const fileName = encodeURIComponent(tag + "+" + mimeType + "+" + String(Date.now()));
        ...
        const commitData = {
            fields: {
                ...
                tags: {
                    mapValue: {
                        fields: Object.fromEntries(Object.entries(tags).map(([key, value]) => [key, { doubleValue: value.toString() }])),
                    },
                },
